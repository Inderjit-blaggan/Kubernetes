...
module "eks" {
  ...
  ## Default mode for vpc cni where each IP is assigned based on per pod request from subnet to the eni of the node, usually a eni can support 30 IPS
    vpc-cni = {
      most_recent = true
    }

  ## Prefixing mode in VPC CNI
    # vpc-cni = {
    #   most_recent              = true
    #   before_compute           = true
